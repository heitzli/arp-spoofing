...
class ArpSpoofer:
    ...
    def get_mac(self, ip):
        ...
        answer = scapy.srp(
            final_packet, iface=self.interface, timeout=2, verbose=False
        )[0]
    ...
    def run(self):
        ...
        try:
            ...
            print(
                Fore.RED + "[!] Detected CTRL+C. Restoring ARP tables... Please wait."
            )
...
if __name__ == "__main__":
    ...
    spoofer = ArpSpoofer(
        target_ip=args.target, spoof_ip=args.spoof, interface=args.interface
    )
    spoofer.run()
